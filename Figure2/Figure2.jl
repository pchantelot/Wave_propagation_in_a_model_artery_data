...
with_theme(My_theme, palette = (color = ColorSchemes.Blues_7, marker = [:circle])) do 
    fig = Figure(size = (502, 675), figure_padding = (1,16,1,1)) 
        
        # Panel 1: Schematic
        panel1 = fig[1,1] = GridLayout()
        Label(panel1[1, 1, TopLeft()], "(a)", padding = (-88, 0, -20, 0), fontsize = 16)
            ax = Axis(panel1[1,1])
            ax.limits = (-27, 64, -8, 25)
            ...
            hidespines!(ax)
                ...
                text!(ax, 49, 11, text = L"\mathrm{Air}", align = (:right, :bottom), fontsize = 16)
                text!(ax, 49, 9, text = L"\mathrm{Water}", align = (:right, :top), fontsize = 16)
                # left
                ...
                band!(ax, z .- 12, fill(9.65,length(z)), 10 .+ y; color = :skyblue1)
                lines!(ax, z .- 12, 10 .+ y, color = :hotpink2, linewidth = 5)
                # deflection
                arrows!(ax, [-12], [9.6], [0], [2.5], linewidth = 2, arrowsize = 10)
                text!(ax, -12.5, 15, text = L"\delta(z)", align = (:right, :center), fontsize = 16)
                # right arrow
                ...
                text!(ax, -20.5, 5, text = L"1 \, \mathrm{cm}", align = (:right, :center), fontsize = 16)
                arrows!(ax, [-5.5], [11.7], [0], [-0.1], linewidth = 2, arrowsize = 12)
                arrows!(ax, [-5.5], [8.3], [0], [0.1], linewidth = 2, arrowsize = 12)
                text!(ax, -5, 10, text = L"h_0", align = (:left, :center), fontsize = 16)
                # camera
                poly!(ax, Rect(23,20, 4, 4), color = :black)
                text!(ax, 28, 22, text = L"\mathrm{Camera}", align = (:left, :center), fontsize = 16)
                ...
                text!(ax, 14, 23, text = L"\mathbf{g}", align = (:left, :center), fontsize = 16)
                # Bottom arrows
                ...
                text!(ax, 25, -6, text = L"58 \, \mathrm{cm}", align = (:center, :center), fontsize = 16)
                arrows!(ax, [-12], [-3.5], [4], [0], linewidth = 2, arrowsize = 12)
                arrows!(ax, [-12], [-3.5], [-4], [0], linewidth = 2, arrowsize = 12)
                text!(ax, -12, -6, text = L"w_0 = 1 \, \mathrm{cm}", align = (:center, :center), fontsize = 16)
                # shaker
                arrows!(ax, [4.5], [10], [0], [2], linewidth = 2, arrowsize = 12)
                arrows!(ax, [4.5], [10], [0], [-2], linewidth = 2, arrowsize = 12)
                text!(ax, 4.5, 15, text = L"\mathrm{Shaker}", align = (:center, :center), fontsize = 16)
                # displacement
                arrows!(ax, [13.25], [10], [0], [1], linewidth = 2, arrowsize = 10)
                text!(ax, 14, 11.5, text = L"u_y(x,z,t)", align = (:left, :bottom), fontsize = 16)
                # Axis 
                arrows!(ax, [2.5, 2.5], [2, 2], [0, 3], [3, 0], linewidth = 1.5, arrowsize = 10)
                text!(ax, 7.7, 1.5, text = L"x", align = (:right, :center), fontsize = 16)
                text!(ax, 2.3, 7, text = L"y", align = (:right, :center), fontsize = 16)
                arrows!(ax, [-9.5, -9.5], [2, 2], [0, -3], [3, 0], linewidth = 1.5, arrowsize = 10)
                text!(ax, -15, 1.5, text = L"z", align = (:left, :center), fontsize = 16)
                text!(ax, -9.2, 7, text = L"y", align = (:left, :center), fontsize = 16)

        # Panel 2: Height Field
        panel2 = fig[2,1] = GridLayout()
        Label(panel2[1, 1, TopLeft()], "(b)", padding = (-48, 0, -5, 0), fontsize = 16)
            ax21 = Axis(panel2[1,1])
            #ax21.alignmode = Outside()
            ax21.ylabel = L"$u_y$ (arb. unit)"
            ...
            hidespines!(ax22)
            colsize!(panel2, 2, Relative(1/16))
            colgap!(panel2, 0)
            bracket!(ax22, 1, 0, 1, 2.5,
                text = L"50 \,\, \mathrm{Hz}", textoffset = 5, width = 10, orientation = :down, 
                color = :black, textcolor = :black)
            bracket!(ax22, 1, 7.5, 1, 5,
                text = L"150 \,\, \mathrm{Hz}", textoffset = 5, width = 10, orientation = :up, 
                color = :black, textcolor = :black)

        panel3 = fig[3,1] = GridLayout()
        Label(panel3[1, 1, TopLeft()], "(c)", padding = (-35, 0, -5, 0), fontsize = 16)
            ax3 = Axis(panel3[1,1])
            #ax3.alignmode = Outside()
            ax3.limits = (0, 1000, 0, 350)
            ax3.ylabel = L"f \,\, \mathrm{(Hz)}"
    ...
    rowsize!(fig.layout, 1, Fixed(170))
    rowsize!(fig.layout, 3, Relative(485/1000))
    rowgap!(fig.layout, 0)
    display(fig)
